...
int main()
{
    ...
    nl_video_analysis::UniformFrameSampler frame_sampler;
    ...
    while (std::chrono::steady_clock::now() - start_time < std::chrono::seconds(30)) {
        ...
        if (clip.has_value()) {
            clip_count++;
            frame_sampler.sampleFrames(clip.value(), 10);
            std::cout << "Received clip #" << clip_count
                     ...
                     << ", sampled frames size :" << clip.value().sampled_frames.size() << std::endl;
